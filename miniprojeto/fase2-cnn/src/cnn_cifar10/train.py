"""Loop de treino/avaliação da CNN, com early stopping e checkpoint automático.

Idêntico ao `mlp_cifar10.train` da Fase 1, exceto por `build_model` (monta a
`CNN` em vez do `MLP`). Mantido assim de propósito: o resto do pipeline
(otimizador, scheduler, loss, early stopping, model-saver) é agnóstico à
arquitetura, então reaproveitar a mesma lógica evita divergência entre as
duas fases.
"""
...
def build_model(config: ExperimentConfig) -> CNN:
    return CNN(
...
def fit(
    ...
    """Treina a CNN conforme `config`, avalia no teste e (opcionalmente) salva a execução.
